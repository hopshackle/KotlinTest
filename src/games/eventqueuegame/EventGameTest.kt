...
fun main() {
    ...
    while (!game.isTerminal()) {
        val redGene = agents[PlayerId.Red]?.getActions(game, 0)?.slice(0..3) ?: listOf(0, 0, 0, 0)
        val redAction = LaunchExpedition(PlayerId.Red, redGene.get(0), redGene.get(1), redGene.get(2), redGene.get(3))
        val blueGene = agents[PlayerId.Blue]?.getActions(game, 1)?.slice(0..3) ?: listOf(0, 0, 0, 0)
